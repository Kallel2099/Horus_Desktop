...
public class PrepararKit extends javax.swing.JDialog {
    ...
    public PrepararKit(LeitorCódP parent, boolean modal) {
        super (parent, modal);
        initComponents();
        Tabela1 = (DefaultTableModel) jTable2.getModel();
    ...
    private void initComponents() {
        ...
        //JTable table = new JTable(lin, col) {
            ...
            jTextFieldCódKt.setBounds(90, 10, 220, 20);

            jLabel1.setText("Código do kit");
            jPanel2.add(jLabel1);
            ...
            jTextFieldNomeKt.setBounds(90, 40, 220, 20);
    ...
    private void jTextFieldCódKtActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jTextFieldCódKtActionPerformed
        ...
        moviment.setQuery("SELECT * FROM movimentação WHERE"
                + " codigo = '"+jTextFieldCódKt.getText().intern().trim()+"' AND status = 'Lavando' AND concluido IS NULL "
                + "OR codigo = '"+jTextFieldCódKt.getText().intern().trim()+"' AND status = 'Pendente' AND concluido IS NULL");
        ...
        if ((instru.getOk() == 1) && (kit.getStatus() == 1)) {
            if (model.getStatus().intern().trim().equalsIgnoreCase("Lavando") 
                ...
                if (model.getStatus().intern().trim().equalsIgnoreCase("Pendente")) {
                    JOptionPane.showMessageDialog(null, "<html><center>Atenção!<br>"
                            ...
                            + "este kit encontra-se com alguma pendencia.", "", JOptionPane.DEFAULT_OPTION);
                }
                jTextFieldCódKt.setEnabled(!true);
                jTextFieldCódKt.setText(instrum.getFantasia().trim());
                ...
                if (kit.getStatus() == 1) {
                    ...
                    kit.setCódKt(jTextFieldCódKt.getText());
                    KitMod modelbd = kt.Buscar(kit);
                    modelbd.setPos(1);
                    for (int i = 0; i < modelbd.getLin(); i++) {
                        modelbd = kt.Buscar(kit);
                ...
                JOptionPane.showMessageDialog(null, "<html><center>O kit não passou"
                        + " pela lavegem.<br>", "", JOptionPane.DEFAULT_OPTION);
                jTextFieldCódKt.setText("");
                jTextFieldCódKt.requestFocus();
            }
            ...
            JOptionPane.showMessageDialog(null, "<html><center>Comece identificando o código"
                    + " do kit.<br>", "", JOptionPane.DEFAULT_OPTION);
            jTextFieldCódKt.setEnabled(true);
            jTextFieldCódKt.setText("");
            jTextFieldCódKt.requestFocus();
            instrum.setOk(0);
    ...
    public static void main(String args[]) {
        try {
            ...
            java.util.logging.Logger.getLogger(PrepararKit.class.getName()).log(java.util.logging.Level.SEVERE, null, ex);
        ...
        java.awt.EventQueue.invokeLater(new Runnable() {
            public void run() {
                PrepararKit dialog = new PrepararKit(pkit, true);
